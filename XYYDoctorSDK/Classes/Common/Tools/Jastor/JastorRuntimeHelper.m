...
static NSString* ivar_getTypeName(Class class, const char* key) {
    unsigned int num;
    NSString* mkey = [NSString stringWithUTF8String:key];
    NSString* mkey1 = [NSString stringWithFormat:@"_%@", mkey];
    Ivar* vars = class_copyIvarList(class, &num);
    NSString* rType = nil;
    for(int i = 0; i < num; i ++) {
        Ivar var = vars[i];
        NSString* cKey = [NSString stringWithUTF8String:ivar_getName(var)];
        if([cKey compare:mkey] == NSOrderedSame || [cKey compare:mkey1] == NSOrderedSame) {
            const char* ctype = ivar_getTypeEncoding(var);
            if(ctype[0] != '@') {
                rType = NSStringFromClass([NSNumber class]);
            }
            else {
                rType = [NSString stringWithUTF8String:ctype];
            }
            break;
        }
    }
...
+ (Class)propertyClassForPropertyName:(NSString *)propertyName ofClass:(Class)klass {
	...
	for (unsigned int i = 0; i < propertyCount; ++i) {
		...
		if (strcmp(cPropertyName, name) == 0) {
            ...
            if(type == nil) {
                NSString* ctype = ivar_getTypeName(klass, property_getName(property));
                if(ctype == nil) {
                    return [NSString class];
                }
                return NSClassFromString(ctype);
            }
