...
@property (nonatomic, strong) TimerHolder *diskCheckTimer;

@property (nonatomic, assign) BOOL userHangup;

@property (nonatomic, strong) TimerHolder *calleeResponseTimer; //被叫等待用户响应接听或者拒绝的时间
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil{
    ...
    if (self) {
        ...
        _diskCheckTimer = [[TimerHolder alloc] init];
...
- (void)startByCallee{
    ...
    _calleeResponseTimer = [[TimerHolder alloc] init];
...
- (void)hangup{
    ...
    if (self.callInfo.localRecording) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            wself.chatRoom = nil;
//            [wself netWenzhenJieshu];
        });
    }
    else {
        self.chatRoom = nil;
//        [self netWenzhenJieshu];
    }
}
