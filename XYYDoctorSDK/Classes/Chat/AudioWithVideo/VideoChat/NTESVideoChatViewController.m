...
- (instancetype)initWithCallInfo:(NTESNetCallChatInfo *)callInfo
{
    ...
    self = [self initWithNibName:@"NTESVideoChatViewController" bundle:sampleBundle];
    if (self) {
...
- (void)initUI
{
    ...
    gradientLayer.colors = @[(__bridge id)[UIColor whiteColor].CGColor, (__bridge id)UIColorFromRGB(0x5EE1D1).CGColor];
    gradientLayer.locations = @[@0, @1.0];
    gradientLayer.startPoint = CGPointMake(0, 0);
    gradientLayer.endPoint = CGPointMake(0, 1);
    gradientLayer.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);//修改layer的大小
    [self.bigVideoView.layer insertSublayer:gradientLayer atIndex:0];
...
//    [ConfigUtil saveInt:1 withKey:DiAGNOSETYPE];
    //防止用户在点了接收后又点拒绝的情况
    if (sender == self.acceptBtn) {
        //接受视频
        [ConfigUtil saveInt:1 withKey:DiAGNOSETYPE];
        WS(weakSelf);
        AcceptVedioCmd *cmd = [[AcceptVedioCmd alloc]init];
        ...
        [[HttpNetwork getInstance] request:cmd success:^(BaseRespond *respond) {
            [MBProgressHUD showSuccess:@"本次问诊已转为视频问诊!" toView:weakSelf.view];
...
//缩小
- (IBAction)onShuoxiao:(id)sender {
    [YLTVideoXuanfuView shareInstance].callInfo=self.callInfo;
    [self dismiss:^{
        [[YLTVideoXuanfuView shareInstance] viewShow];
    }];
}
- (IBAction)onSelfHungUp:(id)sender {
    [super hangup];
    [self dismiss:nil];
}
...
- (void)resetRemoteImage{
    ...
    self.bigVideoView.image = [ImageUtil getImageByPatch:@"netcall_bkg"];
}
