...
#import "XYYChatSDK.h"
//static NTESAudioChatViewController* _instance = nil;
...
//{
//    return [NTESAudioChatViewController shareInstance] ;
//}
//
...
- (void)initUI {
    ...
    if([self.callInfo.caller isEqualToString:[NIMSDK sharedSDK].loginManager.currentAccount]){
        //主叫方是自己
        self.labFromUser.text=self.callInfo.callee;//
    }else{
        self.labFromUser.text=self.callInfo.caller;//请求者
    }
...
//缩小
- (IBAction)onSuoxiao:(id)sender {
    [YLTAudioXuanfuButton shareInstance].callInfo=self.callInfo;
    [self dismiss:^{
        [[YLTAudioXuanfuButton shareInstance] viewShow];
    }];
}
