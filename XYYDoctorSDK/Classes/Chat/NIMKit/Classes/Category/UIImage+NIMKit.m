...
+ (UIImage *)nim_imageInKit:(NSString *)imageName{
    ...
    NSBundle *sampleBundle = [NSBundle bundleWithPath:[NSString stringWithFormat:@"%@/NIMKitResource.bundle",sampleBundlePath]];
    // 3. 从bundle中获取资源路径
    NSString *pic1Path = [sampleBundle pathForResource:[NSString stringWithFormat:@"%@@2x",imageName] ofType:@"png"];
    // 4. 通过路径创建对象
    UIImage *image = [UIImage imageNamed:imageName];
    return image? image :[UIImage imageWithContentsOfFile:pic1Path];
...
+ (UIImage *)nim_emoticonInKit:(NSString *)imageName
{
    ...
    NSString *sampleBundlePath = [FileUtil getSDKResourcesPath];
    // 2. 载入bundle，即创建bundle对象
    NSBundle *sampleBundle = [NSBundle bundleWithPath:[NSString stringWithFormat:@"%@/NIMKitEmoticon.bundle",sampleBundlePath]];
    // 3. 从bundle中获取资源路径
    NSString *pic1Path = [sampleBundle pathForResource:[NSString stringWithFormat:@"%@@2x",imageName] ofType:@"png"];
    // 4. 通过路径创建对象
