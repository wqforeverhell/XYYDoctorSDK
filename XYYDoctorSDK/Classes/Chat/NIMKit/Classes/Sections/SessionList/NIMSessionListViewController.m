...
- (void)didAddRecentSession:(NIMRecentSession *)recentSession
    ...
    WEAK_SELF(weakSelf);
    [[NIMSDK sharedSDK].userManager fetchUserInfos:@[recentSession.session.sessionId] completion:^(NSArray *users, NSError *error) {
        [weakSelf.recentSessions addObject:recentSession];
        [weakSelf sort];
        _recentSessions = [weakSelf customSortRecents:_recentSessions];
        [weakSelf refresh];
    }];
...
- (void)onSelectedRecent:(NIMRecentSession *)recentSession atIndexPath:(NSIndexPath *)indexPath{
    ...
    NIMUser *me    = [[NIMSDK sharedSDK].userManager userInfo:recentSession.session.sessionId];
    vc.strTitle=recentSession.session.sessionId;
    if (!IS_EMPTY(me.userInfo.nickName)) {
        vc.strTitle=me.userInfo.nickName;
    }
    vc.hxAccount = recentSession.session.sessionId;
    vc.isShowChufang = NO;
...
- (NSString *)nameForRecentSession:(NIMRecentSession *)recent{
    NIMUser *me    = [[NIMSDK sharedSDK].userManager userInfo:recent.session.sessionId];
    if (IS_EMPTY(me.userInfo.nickName)) {
        return recent.session.sessionId;
    }
    return me.userInfo.nickName;
